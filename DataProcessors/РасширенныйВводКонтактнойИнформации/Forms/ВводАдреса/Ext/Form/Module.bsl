...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	//ЛБ - н
	Если НЕ ПолучитьСтранаРоссия() Тогда
		Возврат;
	КонецЕсли;
	//ЛБ - к
	
	Если Не СтрДлина(СокрЛП(Индекс)) = 6 Или Индекс = "000000" Или (Не ЛБ_ОбщегоНазначения.СтрокаНеСодержитБукв(Индекс)) Тогда
		Если Не СтрДлина(СокрЛП(Индекс)) = 0 Тогда
...
Процедура КомандаОК(Команда)  
	//ЛБ - н
	Если ПолучитьСтранаРоссия() Тогда
		// ++ Сербаев 11.05.2023
		Если Индекс = "" Тогда
...
Процедура ЗаполнитьПредопределенныеВариантыАдреса(Параметры)
	...
	Если ПустаяСтрока(Помещение) И ПустаяСтрока(ТипПомещения) Тогда 
		...
		//Савранин ++
		ТипПомещения = ТипПомещенияКвартира(Элементы.ТипПомещения);
		//Савранин ++    
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ТипПомещенияКвартира(Знач СписокДанных)
	
	ТипСписка = ТипЗнч(СписокДанных);
	Если ТипСписка = Тип("СписокЗначений") И СписокДанных.Количество() > 0 Тогда
		Возврат СписокДанных.НайтиПоЗначению("Квартира");
	ИначеЕсли ТипСписка = Тип("Массив") И СписокДанных.Количество() > 0 Тогда
		Возврат СписокДанных[0];
	ИначеЕсли ТипСписка = Тип("ПолеФормы") Тогда
		Возврат ТипПомещенияКвартира(СписокДанных.СписокВыбора);
	КонецЕсли;
