...
//+Мельников 26.04.24 Установка префикса инвентаризаций. Пример: ДУ00-161001
Процедура ЛДУ_УстановитьПрефиксОрганизацииИИнформационнойБазыИЛомбарда(Источник, СтандартнаяОбработка, Префикс) Экспорт

	УстановитьПрефикс(Источник, Префикс, Истина, Истина, Истина);
...
//Мельников 26.04.24 Добавлен параметр "УстановитьПрефиксЛомбарда"
Процедура УстановитьПрефикс(Источник, Префикс, УстановитьПрефиксИнформационнойБазы, УстановитьПрефиксОрганизации, УстановитьПрефиксЛомбарда = Ложь)
	...
	//+Мельников 24.04.24 Установка префикса ломбарда в номер документа.
	Если УстановитьПрефиксЛомбарда Тогда

		ПрефиксЛомбарда = "";

		Если ЗначениеЗаполнено(Источник.Ломбард) Тогда
			ПрефиксЛомбарда = Источник.Ломбард.Префикс;
		Иначе
			ПрефиксЛомбарда = "";
		КонецЕсли;

		ДополнитьСтрокуНулямиСлева(ПрефиксЛомбарда, 3);
	КонецЕсли;
	
	ШаблонПрефикса = ?(УстановитьПрефиксЛомбарда, "[ОР][ИБ]-[ЛБ][Префикс]" ,"[ОР][ИБ]-[Префикс]");	
	
	//Было: ШаблонПрефикса = "[ОР][ИБ]-[Префикс]";
	//-Мельников
	ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[ОР]", ПрефиксОрганизации);
	
	//+МАН Добавил проверку тип документа
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.ЛБ_Инвентаризация") Тогда
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[ИБ]", "00");
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ЛБ_ИнвентаризацияДС") Тогда
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[ИБ]", "01");
	//+МАН 28.01.25 Добавил инвентаризацию отходов
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ЛБ_ИнвентаризацияОтходовДМ") Тогда
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[ИБ]", "02");
	//+МАН 16.01.25 Добавил Приход/расход отходов
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ЛБ_ПриходОтходовДМ") ИЛИ ТипЗнч(Источник) = Тип("ДокументОбъект.ЛБ_РасходОтходовДМ") Тогда
		ШаблонПрефикса = "[ЛБ]/[Префикс]";
	//-МАН
	Иначе
	...
	//+Мельников 26.04.24
	Если УстановитьПрефиксЛомбарда Тогда
		ШаблонПрефикса = СтрЗаменить(ШаблонПрефикса, "[ЛБ]", ПрефиксЛомбарда);
	КонецЕсли;
	//-Мельников
