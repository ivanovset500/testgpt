...
Процедура ПубликацияЗапросаКоммерческихПредложенийБидзаар(Знач ПараметрыЗапроса, АдресРезультата) Экспорт
	...
	ЗапросКП = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗапроса, "ЗапросКоммерческихПредложений",
		Документы.ЗапросКоммерческихПредложенийПоставщиков.ПустаяСсылка()); // ДокументСсылка.ЗапросКоммерческихПредложенийПоставщиков
	РеквизитыЗапросаКП = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗапросКП, "Организация, МенеджерEmail, СписокПолучателейЗапроса");
	ПланПубликации = ПланПубликацииЗапросаКоммерческихПредложенийБидзаар(ЗапросКП, РеквизитыЗапросаКП.Организация);
	Если ПланПубликации = Неопределено Тогда
		РезультатПубликации.ТекстОшибки = НСтр("ru = 'Не удалось создать служебные объекты публикации'");
	...
	Если ПараметрыЗапроса.ПубликоватьПрисоединенныеФайлы Тогда
		...
		ПараметрыЗапросаПубликацииДокументов.Вставить("ОписанияФайлов",
			ОписанияФайловДляПубликацииЗапросаКоммерческихПредложенийБидзаар(ЗапросКП));
	...
	Если РезультатПубликации.ТекстОшибки = "" Тогда
		...
		СостояниеЗапросаКП.ТекущееСостояние = СостояниеЗапросаКП.НовоеСостояние;
		СостояниеЗапросаКП.СостояниеВСервисе = ИнтеграцияСЭлектроннымиТорговымиПлощадкамиКлиентСервер.СостояниеЧерновик();
	Иначе
	...
	Попытка
		...
		РезультатПубликации.СостояниеДокумента.СостояниеСинхронизации = ПланПубликации.СостояниеСинхронизации;
	Исключение
		ОтменитьТранзакцию();
	...
	ЗапросОписанияУчастников.УстановитьПараметр("Ссылка", ЗапросКП);
		...
		Если Не РезультатЗапросаВТ.Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Событие = ИнтеграцияСЭлектроннымиТорговымиПлощадкамиКлиентСервер.СостояниеОпубликован()
					...
					Или Событие = ИнтеграцияСЭлектроннымиТорговымиПлощадкамиКлиентСервер.СостояниеСборПредложенийАвтопродление() Тогда
