...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ++ Сербаев 04.07.2023 TODO:
	Если Параметры.Свойство("СохранитьВерсиюМакета") Тогда
	    ЛБ_СохранитьВерсиюМакета = Параметры.СохранитьВерсиюМакета; 
		
		НовыйЭлемент = ЭтаФорма.Элементы.Добавить("ДатаНачалаДействияМакета", Тип("ПолеФормы"), Элементы.Группа3);
		НовыйЭлемент.Вид                       = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.ПутьКДанным               = "ЛБ_ДатаНачалаДействияМакета";
		НовыйЭлемент.АвтоОтметкаНезаполненного = Истина;
	КонецЕсли;
	
	Если Параметры.Свойство("ВерсияМакета") Тогда
	    ТабличныйДокумент = Параметры.ВерсияМакета.ТабличныйДокумент;
		ЛБ_ДатаНачалаДействияМакета = Параметры.ВерсияМакета.ДатаМакета; 
	КонецЕсли;
    // -- Сербаев 04.07.2023 TODO:
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	// ++ Сербаев 05.07.2023 TODO:
	Если ЛБ_СохранитьВерсиюМакета Тогда
	    Возврат;
	КонецЕсли;
    // -- Сербаев 05.07.2023 TODO:
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если ЛБ_СохранитьВерсиюМакета Тогда
		...
		Если ЛБ_ДатаНачалаДействияМакета = Дата(1, 1, 1) Тогда
		    ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указана дата начала действия Макета!'"), , , "ЛБ_ДатаНачалаДействияМакета");
			Возврат;
		КонецЕсли;
		
		ВерсияМакета = СтрШаблон("%1 действует с %2", ИмяДокумента, Строка(Формат(ЛБ_ДатаНачалаДействияМакета, "ДЛФ=ДД")));
	    СохранитьШаблонМакета(ВерсияМакета);
		Закрыть(ВерсияМакета);
		Возврат;
	Иначе
	// -- Сербаев 04.07.2023 TODO:
...
// ++ Сербаев 04.07.2023 TODO:
&НаСервере
Процедура СохранитьШаблонМакета(ВерсияМакета)

	НоваяЗапись = РегистрыСведений.ЛБ_МакетыПечатныхФорм.СоздатьМенеджерЗаписи();
	НоваяЗапись.ИсходныйМакет = ИмяДокумента;
	НоваяЗапись.ВерсияМакета  = ВерсияМакета;
	НоваяЗапись.Период        = ЛБ_ДатаНачалаДействияМакета;
	НоваяЗапись.Макет         = Новый ХранилищеЗначения(ТабличныйДокумент);
